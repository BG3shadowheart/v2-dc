...
async def _download_bytes(session, url, size_limit=HEAD_SIZE_LIMIT, timeout=REQUEST_TIMEOUT):
    try:
        ...
        hdrs = {"Referer": "https://www.pixiv.net/", "User-Agent": "WaifuBot/1.0"}
...
# These lists are kept for reference but NO provider uses them as filters.
# Every provider fetches unrestricted explicit content.
...
# FULLY UNRESTRICTED — every provider fetches rating:explicit with no tag narrowing.
...
async def fetch_safebooru_r(session, positive=None):
    """Safebooru.donmai.us (rule34 variant) — free, no key, gelbooru-compatible."""
...
_PROVIDER_FAIL_LIMIT = 10   # raised from 5 — providers stay active longer
...
async def _fetch_one(session, used_hashes=None):
    ...
    try:
        ...
        # Log when a provider hits the fail limit
...
def _streak_badge(streak: int) -> str:
    """Return a badge string for the streak, or empty string if no badge yet."""
...
def get_join_greeting(member) -> str:
    ...
    _update_streak(uid)  # track daily streak
    ...
    # 40% chance to use holiday greeting when a holiday is active
...
async def send_greeting_embed(channel, session, greeting_text, image_url, member,
    ...
    try:
        ...
        # Retry image download up to 3 times if it fails
        ...
        if not image_bytes or len(image_bytes) > DISCORD_MAX_UPLOAD:
            # Download failed — this should be rare, caller handles fallback
            return False
        ...
        # Build footer: tier + streak badge
        if tier:
        ...
        # Embed variety: 25% chance use as thumbnail (small corner image) instead of full-size
        use_thumbnail = random.random() < 0.25
        if use_thumbnail:
            embed.set_thumbnail(url=f"attachment://{filename}")
        else:
            embed.set_image(url=f"attachment://{filename}")

        # 25% chance to add a random mood line as a field
...
async def rotate_status():
    """Rotate the bot's Discord status every 2 hours."""
    try:
...
async def on_ready():
    ...
    for task in (autosave_task, vc_reconnect_heartbeat,
                 prefetch_pool_filler, random_mood_drop, rotate_status):
    ...
    await _log(
        ...
        f"`{len(PROVIDERS)}` providers active | "
...
async def on_voice_state_update(member, before, after):
    ...
    if now_monitored and (not was_monitored or before.channel.id != after.channel.id):
        ...
        # ── GUARANTEED IMAGE: retry up to 5 times, delay 2s between attempts ──
        gif_url = None
        ...
        if gif_url:
            ...
            if not sent:
                # Image download failed after retries inside send_greeting_embed
                # Try once more with a fresh URL before falling back
            ...
            await _log(
                f"⚠️ **All providers failed** for `{member.display_name}` after 5 attempts "
                f"— sent text greeting instead."
            )
            await _safe_send(channel, content=greeting)
